...
class NEATImprovedPlayer(Player):
    ...
    def create_network_inputs(self, moves, game_board: GameBoard):
        ...
        for move in moves:
            ...
            formatted.append(opponentNum)

            for i in opponentCityColoursLeft:
                formatted.append(i)
        ...
        if not self.has_won():
            ...
            for move in inputs:
                outputs.append(self.neuralNetwork.activate([move[0], move[1], move[2], move[3], move[4], move[5],
                                                            move[6], move[7], move[8], move[9], move[10], move[11]]))
            while True:
                choice = inputs[outputs.index(max(outputs))][12]
                cost = abs(inputs[outputs.index(max(outputs))][11])
                if self.tracksToPlace != -1:
